...
- Expenses (optional, encrypted enablement key). Tabs:
  - Month end: checklist for one month on the year strip. Tidy tags, Reimbursements to ask for before month-end, Closing balances from statements after it, Monthly savings suggestion, Reconcile and optionally close. Never moves money.
  - Budget: plan vs actual for the header period selector. Groups then categories then rows. Reimbursable family held out (tie-out names it). Transfers and “kept out of the family budget” accounts omitted. Plan edits stay on the dashboard.
